# Lifecycle Modeling Language (LML) Certification Overview 
Based on the LML v2.0 specification [https://www.lifecyclemodeling.org/hubfs/LML%20Specification%20v2.0-1.pdf?hsLang=en], the exam is time-boxed to 120 minutes and has seventy (70) multiple-choice questions. Examinees may request training or self-study.

## Exam Breakdown
- xx% requirements
- xx% structure
- xx% behavior
- xx% project management
- xx% risk management
- xx% application

## Who should pursue LML Certification?
- Systems Engineers
- System Analysts
- Product Engineers
- Process Engineers
- Product Owners
- Requirements Engineers
- Mission Engineers
- Agile Release Teams
- Project Managers
- Risk Analysts
- Software Engineers

## Ideal for working professionals looking to:
- Understand the system engineering lifecycle
- Utilize model-based systems engineering techniques 
- Increase speed of product releases

## How to get Certified <!-- This needs to be considered -->

## Training Providers
- Enola Technologies, LLC [https://enola.com/]
- Lifecycle Model-based Systems Engineering (LMBSE) [https://www.lmbse.com/]
- SPEC Innovations [https://specinnovations.com/#]
- Teaching Science and Technology, Inc (TSTI) [https://www.tsti.net/]

## Additional Resources
Vaneman, W. K. (2016). Enhancing model-based systems engineering with the Lifecycle Modeling Language. 2016 Annual IEEE Systems Conference (SysCon), 1–7. [https://doi.org/10.1109/SYSCON.2016.7490581] [https://www.academia.edu/59508641/Enhancing_model_based_systems_engineering_with_the_Lifecycle_Modeling_Language]
